...
ylabel('Amplituda [-]')
axis([0 9 -0.1 0.1])
%% spektrogramy kanałów po filtrowanku
=======
subplot(221)
plot(y1)
subplot(222)
...
%% periodogramy kanałów po filtrowanku
...
subplot(211)
plot(t,y2)
title('Kanał prawy -przed filtracją')
xlabel('Czas [s]')
ylabel('Amplituda [-]')
axis([0 9 -0.3 0.3])
subplot(212)
plot(t,z2)
title('Kanał prawy - po filtracji')
xlabel('Czas [s]')
ylabel('Amplituda [-]')
axis([0 9 -0.1 0.1])
%% 
...
%% autokorelacja (prawy kanał wygryw)
...
%% czesc 3 -- nieeee too  chyba trzeba było
...
%% liczenie odpowiedzi (powinno wyjść to co na F. 7)
Z2(:,1) = ifft(Yf(:,1)'.*(Yp));
Z2(:,2) = ifft((Yp).*Yf(:,2)');
figure(8)
subplot(211)
